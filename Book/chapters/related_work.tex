...
Similarly, Implicit Q-Learning (\acrshort{iql})~\cite{kostrikov2022offline} avoids querying unseen actions altogether during training.
...
In practice, BCQ trains a generative model (e.g.,\ a \acrshort{vae}) to produce candidate actions similar to those in the dataset, ensuring that the policy cannot stray far from logged behavior.
...
Meta-imitation learning (\acrshort{mil}) extends imitation learning into the meta-learning regime, allowing robots to rapidly learn new skills from only a few demonstrations by leveraging experience across many tasks.
...
For example, Model-Agnostic Meta-Learning (\acrshort{maml})~\cite{finn2017} learns a policy initialization that can be quickly adapted to new tasks via gradient descent.
