...
\begin{enumerate}
 \item The first contribution is the provision to the embodied agents community of the ROS4VSNdevelopment, a novel Robot Operating System (ROS)~\cite{ros} based architecture that enables testing and comparing different \acrshort{vsn} models in real robots.
 The ROS4VSNdevelopment is model agnostic; thus, any \acrshort{vsn} solution can be integrated into it with ease.
 ...
 \item Finally, an experimental evaluation of the adapted \acrshort{vsn} models is presented, using the ROS4VSNframework, with two different robots, in a real world scenario (Section~\ref{sec:experiments}).
...
Once the ROS4VSNsystem is available, some state-of-the-art \acrshort{vsn} models need to be selected that will enable the experimental evaluation with real robots.
...
The ROS4VSNlibrary has been designed to be modular and flexible, so that it can be easily adapted to different robots and \acrshort{vsn} models.
...
A complete robot\_api package has been developed to guarantee the compatibility with the rest of ROS4VSNarchitecture.
...
The official ros\_astra\_camera package~\cite{orbeccros} has been adapted to be integrated in the ROS4VSNarchitecture.
The modular design of ROS4VSNallows it to be used with any other type of camera on the robots, simply by adapting this Camera API package.
...
The ROS4VSNpackage acts as a server to which clients can request a set of discrete movements and configure the forward distance or turning angles.
...
Therefore, ROS4VSNmust integrate error control strategies.
...
The ROS4VSNarchitecture continuously measures these errors to adapt the rotation and displacement movements of the robots, ensuring that they occur with the highest possible precision.
...
One of the main features of the ROS4VSNsoftware is that it enables easy integration of different \acrshort{vsn} models independently of the robotic platform used.
...
It is precisely thanks to the ROS4VSNsystem that this can be done, as the technical modifications made to the models are oriented towards embedding them in a ROS-based system.
Next, a detailed description of the modifications and adaptations made to these models is provided so that they can be integrated into ROS4VSNand tested on real robotic platforms.
...
For the experiments presented in this work, the VLV model was embedded in the ROS4VSNarchitecture.
...
This integration into the robots, using the ROS4VSNarchitecture, has consisted of the following steps.
...
These movements are compatible with the developed discrete\_move package of the ROS4VSNarchitecture.
...
These two policies have been integrated into the ROS4VSNnodes, and with them, the robot can explore the environment.
...
Once this BC is finished, a RL fine-tuning is used following the \acrshort{ddppo} approach~\cite{Wijmans2019DDPPOLN}.
...
The action space used for the experiments with this model is: \moveforward 25 cm, \movebackward 25 cm, \turnright $30^\circ$, \turnleft  $30^\circ$ and \stopac.
...
To do so, the ROS4VSNwas embedded in both of these platforms.
...
The modular architecture of the developed ROS4VSNsystem was used to deploy it in a distributed manner.
...
Table~\ref{tab:pirlnav} shows the SR obtained for the PIRLNav model integrated into the ROS4VSNarchitecture.
...
The robots operated without direct assistance throughout the experiments, being automatically operated by the \acrshort{vsn} models integrated into the ROS4VSNsoftware architecture.
...
It is expected, therefore, that the ROS4VSNlibrary plays a fundamental role in this line of research.

Analyzing the particular behavior of the models, the following interesting discussion can be provided.
The integration of the VLV and PIRLNav models within the ROS4VSNarchitecture has proven to be successful.
...
To conclude, a ROS-based framework for visual semantic navigation named ROS4VSNhas been presented.
